<?php
  ...
  $stmt = $pdo->query("SELECT role_id,role_name,effective_from_dt,effective_end_dt
                       FROM role_master
...
//If make role inactive button is clicked
if(isset($_POST['set_inactive'])){
  if(isset($_POST['delete'])){
    $date = date("yy-m-d");
    foreach($_POST['delete'] as $deleteid){
      $sql = "UPDATE role_master
      ...
      $stmt->execute(array(
        ...
        $_SESSION['message']='Selected Roles Set Inactive.';
      ...
      $_SESSION['message']='No Roles are Chosen to Set Inactive.';
...
if(isset($_POST['edit'])) {
  if(isset($_POST['delete'])) {
    foreach($_POST['delete'] as $deleteid){
      $role_name = $_POST['role_name'.$deleteid.''];
    ...
    $_SESSION['message']='No roles are chosen to make changes.';
    $_SESSION['status'] = "warning";
    $_SESSION['title'] = "Warning!";
    header('Location: role_master.php');
    return;
...
//IF THE ADD ROLE BUTTON IS CLICKED
if(isset($_POST['add_role'])) {
    $date = date("yy-m-d");
    $sql = "INSERT INTO role_master
             (role_name,effective_from_dt)
             VALUES
             (:role_name,:effective_from_dt)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  <main class="content">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
      <div class="d-block mb-4 mb-md-0">
        ...
        <h2 class="h4">Role Master</h2>
        <p class="mb-0">Manage all of the roles here</p>
      </div>
      ...
      <div class="row align-items-center justify-content-between">
        <div class="col col-md-6 col-lg-3 col-xl-4">
          <div class="input-group me-2 me-lg-3 fmxw-400">
            ...
            <input type="text" id="myInput" class="form-control" placeholder="Search roles">

            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal-default">Add New Role</button>
    ...
    <!-- Add new role form -->
    <form method="post" action="role_master.php" enctype="multipart/form-data">
    <div class="modal fade" id="modal-default" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h2 class="h6 modal-title">Add New Role</h2>
          ...
          <div class="mb-3">
            ...
            <input type="text" required class="form-control" name="role_name" id="exampleFormControlInput1" placeholder="Role name">
          </div>
          
          </div>
          <div class="modal-footer">
            
          <button type="submit" class="btn btn-success" name="add_role">Add Role</button>
      ...
      <table class="table table-hover">
        <thead>
          ...
          <th scope="col" class="text-center"><i class="fa fa-check" aria-hidden="true"></i></th>
            <th class="border-gray-200">Role Name</th>
        ...
        <tbody id="myTable">
          ...
          <?php foreach($roles as $role): ?>
            ...
            <?php
      if(isset($role['effective_end_dt'])){
        $active = "disabled";
      } else{
        $active = "";
      }
    ?>
          <tr>
            ...
            <th scope="row"><input type="checkbox" class="form-control-xs" name='delete[]' id="role_id<?= $role['role_id'] ?>" value='<?= $role['role_id'] ?>' ></td>
            <td>
            
              <span hidden class="fw-normal"><?= $role['role_name'] ?></span>
              <input class="form-control" type="text" onchange="check('role_id<?= $role['role_id'] ?>')" value="<?= $role['role_name'] ?>" name="role_name<?= $role['role_id'] ?>" <?= $active ?>>
            </td>
            <td>
                <?= $role['effective_from_dt']?>
            </td>
            <td>
                <?= $role['effective_end_dt']?>
            </td>
          </tr>
          <?php endforeach; ?>
          <!-- Item -->
